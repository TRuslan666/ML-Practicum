...
CLASSES_FILE = Path("data/processed/classes.txt")
...
MAX_ASPECT_RATIO = 8.0  # максимально допустимое соотношение сторон бокса (w/h или h/w);
                         # после поворота/скейла знак может вытянуться почти в линию —
                         # такие боксы тоже отбрасываем
...
transform = A.Compose(
    [
        ...
        # Тот же набор, что Ultralytics встраивает в YOLO автоматически "на лету"
        # (видно в логе обучения как строку "albumentations: Blur(p=0.01)...").
        # Faster R-CNN / SSD / EfficientDet / DETR такого встроенного пайплайна
        # не имеют, поэтому "запекаем" то же самое здесь — иначе сравнение
        # моделей будет нечестным (YOLO получает доп. аугментацию бесплатно,
        # остальные — нет).
...
def main():
    ...
    print(f"Будет сгенерировано копий на каждую: {MULTIPLIER}")
    ...
    for img_path in tqdm(image_paths, desc="Аугментация"):
        ...
        for i in range(1, MULTIPLIER + 1):
            ...
            # ДОЛЮ видимой площади бокса, а не его абсолютный размер в пикселях.
            # После поворота/скейла бокс может остаться формально "видимым", но
            # сжаться до 1 пикселя или вытянуться в линию — отбрасываем такие отдельно.
            filtered_bboxes, filtered_classes = [], []
            for (cx, cy, w, h), cls_id in zip(aug_bboxes, aug_classes):
                # КЛЮЧЕВАЯ ПРОВЕРКА (должна идти ПЕРВОЙ): NaN/Inf в координатах.
                # В Python "nan < X" и "nan > X" всегда возвращают False, поэтому
                # проверки размера/aspect ratio ниже НЕ отлавливают NaN-боксы —
                # они тихо проходят все фильтры и попадают в .txt как буквальная
                # строка "nan", что приводит к illegal memory access на GPU.
                coords = (cx, cy, w, h)
            ...
            # ВАЖНО: сохраняем всегда в .jpg, независимо от формата исходника.
            # Ultralytics YOLO не распознаёт .ppm как формат изображения —
            # если сохранить аугментированную копию с суффиксом ".ppm"
            # (как было раньше через img_path.suffix), YOLO её просто не увидит,
            # хотя файл физически будет на месте.
            new_img_path = TRAIN_IMAGES_DIR / f"{new_stem}.jpg"
